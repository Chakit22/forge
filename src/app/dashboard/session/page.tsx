...
export default function Session() {
    ...
    Array.from(files).forEach((file) => {
      ...
      reader.onload = (event) => {
        if (event.target && event.target.result) {
          ...
          console.log(`File ${file.name} loaded, content length: ${content.length}`);
  ...
  const handleSendMessage = async () => {
    ...
    const displayContent = attachments.length > 0 
      ? `${userMessage.content} [Attached ${attachments.length} file(s): ${attachments.map(a => a.name).join(', ')}]`
      : userMessage.content;
    ...
    if (attachments.length > 0) {
      ...
      setMessages(prev => [
        ...
        {
          role: "assistant",
          content: "I'll try to process your file(s). If you don't see file content in my response, there might be an issue with file processing. You may need to try again with a smaller or different file format."
        }
      ]);
    } else {
    ...
    try {
      ...
      if (attachmentData.length > 0) {
        console.log('Attachment details:', attachmentData.map(a => ({
          name: a.name,
          type: a.type,
      ...
      <main className="flex-1 flex flex-col p-4 md:p-8">
        <div className="flex-1 mb-4 overflow-auto bg-teal-700/50 rounded-lg p-4">
          <div className="flex flex-col gap-4">
            <div className="self-start bg-teal-600/70 text-white rounded-lg p-3 max-w-md">
                ...
                {msg.role === "assistant" ? (
                  <div className="prose prose-invert prose-sm max-w-none">
                    <ReactMarkdown 
                      ...
                      components={{
                        a: (props) => (
                          <a {...props} className="text-teal-300 underline hover:text-teal-200" target="_blank" rel="noopener noreferrer" />
                        ),
                        ul: (props) => (
                        ...
                        code: ({ inline, ...props }: CodeProps) => (
                          inline 
                            ? <code {...props} className="bg-teal-700/50 px-1 py-0.5 rounded text-sm" />
                            : <code {...props} className="block bg-teal-700/70 p-2 rounded-md overflow-x-auto text-sm my-2" />
                        ),
                        pre: (props) => (
                        ...
                        blockquote: (props) => (
                          <blockquote {...props} className="border-l-2 border-teal-400 pl-3 italic" />
                        ),
                      }}
                    >
                      {msg.content}
          ...
          <div className="mb-2 p-2 bg-teal-700/30 rounded-lg">
            ...
            <div className="flex flex-wrap gap-2">
              {attachments.map((file) => (
                <div key={file.id} className="flex items-center bg-teal-600/60 rounded px-2 py-1">
