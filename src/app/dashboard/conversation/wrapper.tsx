...
export default function WeaviateConversationWrapper({
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      // @ts-expect-error - Using dynamic window property to track conversation events
      window.__conversationEventsDispatched = window.__conversationEventsDispatched || new Set();
    }
    
    console.log(`Wrapper mounted for conversation ${conversationId}`);
    const hasDispatched = typeof window !== 'undefined' && 
    ...
    const loadMessages = async () => {
      try {
        ...
        try {
          const savedData = localStorage.getItem(`conversation_${conversationId}_messages`);
          if (savedData) {
            ...
            if (messages.length > 0 && !hasDispatched) {
              ...
              console.log('Dispatching load-messages event from localStorage backup (first time ever)');
  ...
  useEffect(() => {
    ...
    window.addEventListener('save-messages', handleMessageSave as EventListener);
    
    // Cleanup
    return () => {
      window.removeEventListener('save-messages', handleMessageSave as EventListener);
    };
  }, [conversationId]);
